...
func NewClaudeCollector(homeDir string, showCost bool, pricingCfg config.PricingConfig) *ClaudeCollector {
	...
	return &ClaudeCollector{
		homeDir:      homeDir,
		showCost:     showCost,
		pricingTable: EffectivePricing(pricingCfg),
	}
}
...
func (c *ClaudeCollector) Collect(dr model.DateRange) ([]model.Activity, error) {
	...
	// Key: "sessionID:project:YYYY-MM-DD" — one accumulator per session/project/day so
	// sessions that span midnight are attributed to the correct calendar day rather than
	// having all their cost roll up to whichever day firstSeen falls on.
	...
	for _, f := range jsonlFiles {
		if err := c.scanFile(f, dr, loc, accums, assistantMsgs); err != nil {
			continue
		}
	}

	return c.buildActivities(accums, assistantMsgs, loc), nil
}
...
func (c *ClaudeCollector) scanFile(path string, dr model.DateRange, loc *time.Location, accums map[string]*projectAccum, assistantMsgs map[string]deferredMsg) error {
	f, err := os.Open(path)
	if err != nil {
		return err
	}
	...
	for scanner.Scan() {
		...
		day := rec.Timestamp.In(loc).Format("2006-01-02")
		key := rec.SessionID + ":" + project + ":" + day
		pa, ok := accums[key]
		if !ok {
...
func (c *ClaudeCollector) buildActivities(accums map[string]*projectAccum, assistantMsgs map[string]deferredMsg, loc *time.Location) []model.Activity {
	for _, dm := range assistantMsgs {
		...
		// Collect all qualifying text blocks in document order.
