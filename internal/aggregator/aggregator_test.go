...
func TestAggregateCacheTokens(t *testing.T) {
	now := time.Now()
	...
	tests := []struct {
		...
		{
			name: "single activity cache tokens propagate to ByModel",
			activities: []model.Activity{
				{
					Source:     "claude-code",
					Project:    "proj",
		...
		{
			name: "two activities for same model sum cache tokens",
			activities: []model.Activity{
				{
					Source:    "claude-code",
					Project:   "proj",
		...
		{
			name: "activity with empty model does not create ByModel entry",
			activities: []model.Activity{
				{
					Source:     "git",
					Project:    "proj",
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			s := Aggregate(tt.activities)
			if !tt.wantEntryExists {
				if len(s.ByModel) != 0 {
					t.Errorf("expected no ByModel entries, got %v", s.ByModel)
				}
				return
			}
			m, ok := s.ByModel[tt.model]
			if !ok {
				t.Fatalf("expected ByModel entry for %q, not found", tt.model)
			}
			if m.CacheRead != tt.wantCacheRead {
				t.Errorf("CacheRead: got %d, want %d", m.CacheRead, tt.wantCacheRead)
			}
			if m.CacheWrite != tt.wantCacheWrite {
				t.Errorf("CacheWrite: got %d, want %d", m.CacheWrite, tt.wantCacheWrite)
			}
		})
	}
}
